...
async function getData(limit: number) {
  ...
  const getBlocks = Array.from({ length: limit }, (_, y) => provider.getBlock(blockNumber - (y + 1)))
  const blocks = await Promise.all(getBlocks);

  const transactionList = blocks[0]?.transactions.slice(0, limit) as string[];
  const getTransactions = transactionList.map(tx => provider.getTransaction(tx))
...
const formatDate = (date: number) => new Date(date * 1000).toLocaleDateString("en", {
  hour: "2-digit",
  minute: "2-digit",
  second: "2-digit",
})
...
const roundNumber = (value: ethers.BigNumberish) => parseFloat(ethers.formatEther(value)).toFixed(4)
      ...
      <div className={styles.grid}>
        <div className={styles.box}>
          ...
          <ul>
            ...
            (<li key={block?.hash}>
              <div>
                <Link href={`${baseUrl}/block/${block?.number}`} target="_blank">
              ...
              <div>
                Validater by:{' '}
                <Link href={`${baseUrl}/address/${block?.miner}`} target="_blank">
                  {shortenWallet(block?.miner)}
                </Link>
                <br />
                <Link href={`${baseUrl}/txs?block=${block?.number}`} target="_blank">
                  {block?.length} txns
                </Link>
              </div>
            </li>)
        ...
        <div className={styles.box}>
          <h3 className={styles.boxTitle}>Last Transactions ({transactions.length})</h3>
          <ul>
            {transactions?.map(tx =>
            (<li key={tx?.hash}>
              ...
              <div>
                {roundNumber(tx?.value as bigint)}
              </div>
            </li>)
            )}
          </ul>
